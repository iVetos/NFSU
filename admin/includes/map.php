...
include_once (__DIR_CLASSES . 'class.map.php');
include_once (__DIR_CLASSES . 'class.image.php');
$action = (isset($_GET['action'])) ? $_GET['action'] : 'show';
$id = (isset($_GET['id'])) ? Db::prepare($_GET['id']) : 0;

// Vars
define('__CONST_SOME_NAME', 'Отделения самбо');
define('__CONST_SOME_TABLE', 'map');
define('__CONST_SOME_DIR', 'img/map/');
...
switch ($action)
{
    ...
    case 'show':
        $smarty->assign('some', Map::show(__CONST_SOME_TABLE));
        break;
    ...
    case 'add':
        if (isset($_POST['name']))
        {
            ...
            if (Map::add(__CONST_SOME_TABLE, $name, $map, $text_s, $text, $img, $title, $description, $keywords))
            {
                $id = Db::returnId();
    ...
    case 'edit':
        if (isset($_POST['name']))
        {
            ...
            if (isset($_FILES) && !empty($_FILES) && !empty($_FILES['img']['name']))
            {
                ...
                $cover = Map::returnCover($id, __CONST_SOME_TABLE);
                if(!empty($cover))
                {
            ...
            {
                $img = Map::returnCover($id, __CONST_SOME_TABLE);
            }

            // Изменяем запись в БД
            if (Map::edit($id, __CONST_SOME_TABLE, $name, $map, $text_s, $text, $img, $title, $description, $keywords))
            {
        ...
        else
        {
            ...
            $some = Map::returnOne($id, __CONST_SOME_TABLE);
            if (!empty($some))
            {
    ...
    case 'del':
        $cover = Map::returnCover($id, __CONST_SOME_TABLE);
        if (Map::del($id, __CONST_SOME_TABLE))
        {
